...
with st.sidebar:
    ...
    st.caption("Rules are optional but strongly recommended. They improve naming + warranty consistency when scraping is weak.")
...
def run_cmd(cmd, cwd):
    ...
    for line in p.stdout:
        ...
        placeholder.text("".join(out_lines[-80:]))
    rc = p.wait()
    if rc != 0:
...
def write_uploaded_or_default(upload, default_path: Path, out_path: Path):
    if upload:
...
if run:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if use_repo_rules:
            ...
            if repo_warranty.exists():
                shutil.copy(repo_warranty, warranty_rules_path)
            else:
                write_uploaded_or_default(warranty_rules_upload, Path(""), warranty_rules_path)

            if repo_series.exists():
                shutil.copy(repo_series, series_rules_path)
            else:
                write_uploaded_or_default(series_rules_upload, Path(""), series_rules_path)
        else:
            write_uploaded_or_default(warranty_rules_upload, Path(""), warranty_rules_path)
            write_uploaded_or_default(series_rules_upload, Path(""), series_rules_path)

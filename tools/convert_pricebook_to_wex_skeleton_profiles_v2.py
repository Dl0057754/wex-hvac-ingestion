...
def extract_from_xlsx(path: Path, brand: str, mapping: Optional[Dict]) -> List[ExtractedItem]:
    ...
    for spec in sheet_specs:
        ...
        skipped_rows: List[Tuple[str,int,str,str]] = []  # (sheet, row, reason, preview)  # (model, context)
            found_prices: Dict[str, str] = {}         # model -> price
...
for m in candidates:
                    mm = norm(m)
                    # reject obvious non-model junk (pure digits, short)
                    if mm.isdigit():
                        continue
                    ctx = txt[:160]

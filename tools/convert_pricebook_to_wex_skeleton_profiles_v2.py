...
def extract_from_xlsx(path: Path, brand: str, mapping: Optional[Dict]) -> List[ExtractedItem]:
    ...
    for spec in sheet_specs:
        ...
        # Keep a record of rows we couldn't interpret (written into the output workbook)
        skipped_rows: List[Tuple[str, int, str, str]] = []  # (sheet, row, reason, preview)
        ...
        for r in range(1 + skip_rows, max_r + 1):
            ...
            # Extract models + row price
            found_models: List[Tuple[str, str]] = []  # (model, context)
            found_prices: Dict[str, str] = {}         # currently uses '_row'
            ...
            def scan_cell_for_model_and_price(v, cidx: int) -> None:
                ...
                candidates: List[str] = []
                ml = MODEL_LABEL_RE.search(txt.upper())
                if ml:
                    candidates.append(ml.group(1))
                for rx in MODEL_RE_LIST:
                    candidates.extend(rx.findall(txt.upper()))

                for m in candidates:
                    mm = norm(m)
                    # reject obvious junk
                    if len(mm) < 6 or mm.isdigit():
                        continue
                    found_models.append((mm, txt[:160]))
            ...
            if not found_models:
                if row_texts:
                    preview = " | ".join(row_texts)[:200]
                    skipped_rows.append((sname, r, "no_model_detected", preview))
                continue
